...
- `question` — sentence with a blank (`___`) for government, case_government, preposition_case, and preposition_meaning exercises, or `"verb → Partizip II"` for partizip exercises
- `topic` — verb in infinitive, except for `preposition_case`/`preposition_meaning` where it's the preposition itself (see below)
- `category` — `government`, `case_government`, `preposition_case`, `preposition_meaning`, or `partizip_ii` (see below); together with `topic` forms the key `Teacher` uses for repetition scheduling, so exercises of different categories for the same verb (or preposition) are scheduled independently
- `answer` — correct answer
- `distractors` — list of wrong options. 3 for government, case_government, and preposition_case exercises; empty (`[]`) for partizip and preposition_meaning exercises (empty list triggers text input instead of multiple choice)
...
- `description` — optional dict with `ru` and `en` keys (both required if present, unlike `hint` which allows a subset): a translation of the target sentence, shown *before* answering (unlike `explanation`, which shows after) via `Locale.description_prompt`. Currently only used by `preposition_meaning` — see below for why.

`topic` — verb in infinitive that the exercise is about (e.g. `"sprechen"`, `"sich freuen"`). Reflexive verbs include `sich`. Exercises for the same verb but different prepositions (`sprechen mit`, `sprechen über`) share the same `topic: "sprechen"` — intentionally, so that all forms of the verb are shown together when the user makes a mistake. Exception: for `preposition_case`/`preposition_meaning`, `topic` is the preposition itself (e.g. `"mit"`, `"für"`) — there is no single governing verb, since the preposition's case/meaning is fixed independently of whatever verb happens to appear in the example sentence.
...
**`preposition_meaning`** — the mirror image of `preposition_case`: the noun is already in its (fixed) case, and the blank is the preposition itself — tests *meaning* ("which preposition means 'through'"), not case. `answer` is a bare preposition word; `distractors: []` (typed input, not multiple choice — see below for why). Because the verb is free/compositional (`fahren`, `gehen`, not an idiom like `government`'s), several same-case prepositions are often still *grammatically* valid substitutes, just with a different meaning (`"Der Zug fährt um den Tunnel"` — drives around the tunnel — is fine German, just not what's being tested) — so the usual sanity check ("distractor must not be grammatically valid") doesn't hold here by itself. `description` (see field list above) closes this gap: it gives the exact target meaning as a translation, so exactly one preposition is correct *for that specific meaning*, even though several would parse. Rendered via `Locale.description_prompt` (a fixed instruction, e.g. `"💭 {description} Введи только пропущенное слово."`) — worded generically ("missing word", not "preposition") in case a future category reuses the same `description`+typed-blank mechanism for something else. No `recalls` for this category: the main question is already a production task (typed, not multiple-choice), so a separate recall step wouldn't add the recognition→production step-up it provides for the multiple-choice categories — and would face the same context-loss disambiguation problem as the main question, without `description` to fix it (recall's own `question` field has no per-language variants to carry a translation).
