...
`Teacher` optionally takes a `chained_categories: dict[str, list[str]]` mapping (loaded by `load_chained_categories()`, gitignored `data/chained_categories.yaml` — file absent means an empty mapping, the feature is simply off). It lets one category "pull forward" another for the same topic once its own exercise is answered: `check_answer()` calls `_expedite_chained_categories()`, which looks up `chained_categories.get(exercise.category, [])` and, for each target category with a matching `topic:target_category` key that already has a *future* due date, brings that due date to today — never pushes it back, never creates a new schedule entry (a never-scheduled key is already due via the existing `date.min` fallback, nothing to do), and is a no-op if no exercises exist under that key at all. This runs regardless of whether the answer was correct, and only affects *when* the target becomes eligible — once picked, it goes through the exact same `next_exercise()`/`check_answer()` path as anything else, no separate state. The category names themselves stay opaque to the code — `chained_categories` is just data, so the content repo can declare new chains without a code-repo change, same reasoning as `category` itself being a free-form string (see intro).
