...
def test_validate_accepts_a_correctly_signed_payload(
    ...
    assert validate_telegram_login(payload, telegram_bot_token) == telegram_user_id


def test_validate_rejects_a_field_tampered_with_after_signing(
    telegram_bot_token: str, telegram_login_payload_factory: TelegramLoginPayloadFactory
) -> None:
    payload = telegram_login_payload_factory()
    payload["id"] = "999"

    with pytest.raises(InvalidTelegramLoginError):
        validate_telegram_login(payload, telegram_bot_token)
...
def test_validate_rejects_a_missing_hash(
    ...
    payload = {k: v for k, v in telegram_login_payload_factory().items() if k != "hash"}
    with pytest.raises(InvalidTelegramLoginError):
        validate_telegram_login(payload, telegram_bot_token)
